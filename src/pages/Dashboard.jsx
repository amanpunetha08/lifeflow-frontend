...
export default function Dashboard() {
  ...
  const sparkData = [{ v: 50 }, { v: 80 }, { v: 120 }, { v: 90 }, { v: 180 }, { v: 250 }];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="rounded-2xl p-6 bg-[#1a1a2e] border border-[#2a2a3e]">
          ...
          <div className="flex items-center gap-4">
            <div className="w-20 h-20">
              <ResponsiveContainer>
                <PieChart>
                  <Pie data={donutData} innerRadius={25} outerRadius={35} dataKey="value" strokeWidth={0}>
        ...
        <div className="rounded-2xl p-6 bg-[#1a1a2e] border border-[#2a2a3e]">
          ...
          <p className="text-2xl font-bold text-emerald-400">+250 XP</p>
        ...
        <div className="rounded-2xl p-6 bg-[#1a1a2e] border border-[#2a2a3e]">
          ...
          <p className="text-4xl font-bold text-white">{user?.discipline_score || 82}<span className="text-lg text-gray-400">/100</span></p>
          <p className="text-emerald-400 text-sm mt-1">Great job! 🎉</p>
        </div>
      </div>
          ...
          <div className="flex gap-2 mb-4">
            {['all', 'todo', 'in_progress', 'completed'].map((t) => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-3 py-1.5 rounded-lg text-sm ${tab === t ? 'bg-indigo-500/20 text-indigo-400' : 'text-gray-400 hover:text-white'}`}>
          ...
          <div className="space-y-2">
            ...
            {filtered.map((task) => (
              <div key={task.id} className="flex items-center gap-3 p-3 rounded-xl bg-[#0f0f1a] border border-[#2a2a3e]">
                <input type="checkbox" checked={task.status === 'completed'} onChange={() => completeTask(task.id)}
            ...
            {monthDays.map((day) => (
              <span key={day.toISOString()}
                className={`w-8 h-8 flex items-center justify-center rounded-full text-xs ${
                  isToday(day) ? 'bg-indigo-500 text-white' : isSameMonth(day, currentMonth) ? 'text-gray-300 hover:bg-white/5' : 'text-gray-600'
                }`}>
                {format(day, 'd')}
              </span>
            ))}
          </div>
          <h4 className="text-sm font-medium text-white mt-6 mb-3">Upcoming Events</h4>
